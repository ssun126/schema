...
Schema.init = function () {
    ...
    /**
     * Schema.define() - Define a schema.js module
     * @param  {string} name
    ...
    api.define = function (name, factory) {
        ...
        if (!module) return;
        // If running in Schema app, subscribe to design/preview events
        if (api.env()) {
    ...
    /**
     * Schema.require() - Load a Schema.js module
     * @param  {string} name
     * @return {object}
     */
    ...
    /**
     * Schema.push() - Add a ready handler into secondary queue
    ...
    /**
     * Schema.env() - Get the state of the Schema app
    ...
    /**
     * Schema.validClick() - validate click target against current touch target
    ...
    /**
     * Schema.resize, Schema.scroll - throttled event proxies
     */
    var resizeEvents = 'resize.schema orientationchange.schema load.schema';
    var scrollEvents = 'scroll.schema ' + resizeEvents;
    ...
    // Schema.location() - Wrap window.location in api
    api.location = function (url) {
        window.location = url;
    };

    // Schema.app - Designer-specific methods
    ...
    if (api.app) {
        ...
        // Schema.location - Re-route location change to trigger an event
        api.location = function (url) {
            window.dispatchEvent(new CustomEvent('__wf_location', { detail: url }));
        };
    }

    // Schema.ready() - Call primary and secondary handlers
    api.ready = function () {
        domready = true;
    ...
    /**
     * Schema.load() - Add a window load handler that will run even if load event has already happened
     * @param  {function} handler
     */
    var deferLoad;
    ...
    // Schema.destroy() - Trigger a cleanup event for all modules
    ...
    /*!
     * Schema._ (aka) Underscore.js 1.6.0 (custom build)
     * _.each
     * _.map
     * _.find
     * _.filter
...
/**
 * ----------------------------------------------------------------------
 * Schema: Multi-language
 */

Schema.define('lang', function ($, _) {
    //'use strict';
    ...
    api.ready = function () {
        ...
        if (!Schema.lang()) return;

        countryCode = Schema.lang().toUpperCase();
    ...
    function init() {
        ...
        MULTI_LANGUAGE_DATA = schemaCommon.multiLanguage.get();
...
Schema.define('ix', function ($, _) {
    ...
    function init() {
        ...
        if (anchors.length) {
            Schema.scroll.on(scroll);
            setTimeout(scroll, 1);
        }

        // Handle loads or readys if they exist
        if (loads.length) Schema.load(runLoads);
        ...
        _.each(triggers, function (trigger) {
            ...
            if (type == 'click') {
                $el.on('click' + namespace, function (evt) {
                    ...
                    if (!Schema.validClick(evt.currentTarget)) return;
    ...
    // Events used by other schema modules
    var events = {
        reset: function (i, el) {
            el.__wf_intro = null;
        },
 ...
 * Schema: Touch events
 */
Schema.define('touch', function ($, _) {
    //'use strict';
...
Schema.define('forms', function ($, _) {
    //'use strict';
    ...
    function build(i, form) {
        ...
        $("select", formEL).each(function (i, el) {
            ...
            width = schemaCommon.fncS.cssStyleGet(cssName, className, "width") || null;
            className = "";
            if (width == null) {
                for (var i = 0; i < classNames.length; i++) {
                    if (classNames[i] != "") {
                        if (width == null) {
                            width = schemaCommon.fncS.cssStyleGet(cssName, "." + classNames[i], "width") || null;
                        }
                    }
                    //className += "." + classNames[i];
                }
            ...
            if ($selectEL.attr("input-type") == "multiple") {
                ...
                if (dataSetID != null) {
                    ...
                    if (onClick == 'y') {
                        ...
                        $("#" + newID).on('open', function (event) {
                            ...
                            $.when(Schema.require('dataSet').requestData(dataSetID.toLowerCase())).then(function (data) {
                    ...
                    if (schemaConfig.mobile) {
                        ...
                        schemaCommon.comboGridBox.set($selectEL, { multiSelect: true, checkboxes: true, selectedIndex: 0, width: width, height: height, dropDownHeight: itemCount * ($($selectEL).height() < 18 ? 32 : $($selectEL).height()) },
                ...
                if (dataSetID != null) {
                    ...
                    if (schemaConfig.mobile) {
                        ...
                        schemaCommon.comboGridBox.set($selectEL, { selectedIndex: 0, width: width, height: height, dropDownHeight: itemCount * ($($selectEL).height() < 18 ? 32 : $($selectEL).height()) },
                            { local: local, textName: "text", valueName: "value", gridStyle: { color: "#777B8C" }, gridClass: $selectEL.attr("class") });
                    }
                    //schemaCommon.comboGridBox.set($selectEL, { selectedIndex: 0, width: width, height: height, dropDownHeight: itemCount * ($($selectEL).height() == 16 ? 31 : $($selectEL).height()) },
                    //    { local: local, textName: "text", valueName: "value",gridStyle:{color:"#777B8C"}, gridClass: $selectEL.attr("class") },
                    //    {
    ...
    function addListeners() {
        ...
        // Handle form submission for Schema forms
    ...
    function submitForm(data) {
        ...
        Schema.require('dataSet').request(action, payload);
        afterSubmit(data);
    };
...
Schema.define('splitter', function ($, _) {
    //'use strict';
    ...
    function build(i, splitter) {
        ...
        for (var i = 0; i < splitterEL.length; i++) {
            ...
            if (splittertype != "col") {
                ...
                height = schemaCommon.fncS.cssStyleGet(cssName, className, "height") || null;
                className = "";
                if (height == null) {
                    for (var i = 0; i < classNames.length; i++) {
                        if (classNames[i] != "") {
                            if (height == null) {
                                height = schemaCommon.fncS.cssStyleGet(cssName, "." + classNames[i], "height") || null;
                            }
                        }
                        //className += "." + classNames[i];
                    }
                ...
                $(el).on("resize", function (event) {
                    ...
                    //var charts = Schema.require("dataSet").charts;
                    ...
                    for (var i = 0; i < charts.length; i++) {
                        ...
                        if (chart.closest('[splitter="col"]').length > 0) {
                            ...
                            if (chart.closest("ul").children().length > 0) {
                                ...
                                var cssWidth = schemaCommon.fncS.cssStyleGet($("html").attr("data-site"), "." + chart.closest("li").attr("class"), "width") || "";
        ...
        //                            // Avoid late clicks on touch devices
        //                            if (!Schema.validClick(e.currentTarget)) return;
...
Schema.define('fileInput', function ($, _) {
    //'use strict';
    ...
    function build(i, splitter) {
        // Store form state using namespace
        var splitterEL = $(splitter);
        var bindList = schemaCommon.bindSetList();
        ...
        schemaCommon.fileInput.set(splitterEL, options);
    }
    return api;
});
 ...
 * Schema: Interactions
 */
Schema.define('dataSet', function ($, _) {
    //'use strict';
        ...
        _.each(list, function (item) {
            ...
            if (el && el.css('display') != "none") {
                if (bindTable[item.id]) {
                    ...
                    //setInterval("Schema.require('dataSet').request('" + item.id + "')", parseInt(item.refreshCycle) * 1000 * 10);
                    if (item.refreshCycle) {
                        try {

                            if (Number(item.refreshCycle)) {
                                setInterval(function () {
                                    Schema.require('dataSet').request(item.id);
    ...
    api.reload = function (dataSetID, timer, flag) {
        if (flag) {
            typeof parseInt(timer) === "number" && setInterval("Schema.require('dataSet').request('" + dataSetID + "')", parseInt(timer) * 1000);
        }
    };
    ...
    api.requestByID = function (id) {
        ...
        var payload = Schema.require('forms').getFormData();
    ...
    api.request = function (dataSetID, params) {
        if (!dataSetID) return;
        var payload = params;
        payload || (payload = Schema.require('forms').getFormData());
    ...
    function fetchContent(dataSet, prm) {
        ...
        var apiUrl = Schema.agentURL + "/Agent/GetComponentInfo?id=" + dataSet.agentId + "&GUBUN=" + prm.GUBUN + "&Type=" + prm.Type + "&page=" + 1 + "&rows=" + 0;
            ...
            if (paramsObj == "{}" || paramsObj == '{"undefined":""}') {
                ...
                return $.get(dataSet.url).done(function () {
                    ...
                    if (dataSet.objectName) {
                        ...
                        retouch.result["rows"] = schemaCommon.fncS.hierarchyReturn(data, dataSet.dataObjectName);
                        retouch.success = true;
                    } else {
                        ...
                        switch (dataSet.methodType.toLowerCase()) {
                            case "get": //호출되지 않는 로직이라고 판단됨.. bingxu. 2016.04.22
                                return $.get(dataSet.url, paramsObj).done(function () {
                                    ...
                                    //if (dataSet.objectName) {
                                    //var value = eval("data." + dataSet.objectName);  // return 변수를 retouch
                                    //var value = schemaCommon.restFul.mapping(data, dataSet.colModelsName, dataSet.dataObjectName);
                                    var value = null;
                                    if (dataSet.colModelsName) {
                                        value = schemaCommon.restFul.mapping(data, dataSet.colModelsName, dataSet.dataObjectName);
                                    } else {
                                        if (dataSet.dataObjectName) {
                                            //value = data[dataSet.dataObjectName];
                                            value = schemaCommon.fncS.hierarchyReturn(data, dataSet.dataObjectName);
                                        } else {
                                            value = data;
                                        }
                                    }
            ...
            if (value && el) {
                switch (item.tag) {
                    ...
                    case "ul":
                        ...
                        //        }
                        //    }
                        //    Schema.require('dataSet').request("all", {line:"line"});
                        //});
                        //break;
                }
            }
    ...
    function checkboxdataSet(bindInfo, dataSetID) {
        ...
        var data = schemaCommon.fncS.hierarchyReturn(options.rows, options.hierarchName) || options.rows;
        schemaCommon.checkBox.set($native, { local: data, textName: options.textName, valueName: options.textName });
    }
    function radiodataSet(bindInfo, dataSetID) {
        ...
        var options = JSON.parse(bindInfo.optionScript);
        var data = schemaCommon.fncS.hierarchyReturn(options.rows, options.hierarchName) || options.rows;
        schemaCommon.radioButton.set($native, { local: data, textName: options.textName, valueName: options.textName });
    }
    function jqGridSet(bindInfo, dataSetID) {
        ...
        options.loadComplete = function (data) {
            var table = this;
            merge = schemaCommon.fncS.rangeNum(merge);
            $.each(merge, function (i, v) {
                console.log(v);
                var vSplit = v.split("~");
                if (vSplit.length > 1) {
                    schemaCommon.grid.rowspan($(table), Number(vSplit[0]), Number(vSplit[1]));
                } else {
                    schemaCommon.grid.rowspan($(table), Number(vSplit[0]));
                }

            });
            //schemaCommon.grid.rowspan($(table), 0,2);
            setTimeout(function () {
                //table 버튼아이콘. sjjo
        ...
        //Muti-language 필요시
        //options.colModel = Schema.require('lang').apply(options.colModel) || options.colModel;
        for (var i = 0; i < options.colModel.length; i++) {
...
Schema.define('maps', function ($, _) {
    ...
    api.preview = function () {
        ...
        Schema.resize.off(triggerRedraw);
        if ($maps.length) {
            Schema.resize.on(triggerRedraw);
            triggerRedraw();
        }
    };
    ...
    function triggerRedraw() {
        if ($maps.length && Schema.app) {
            $maps.each(Schema.app.redrawElement);
        }
    }
    ...
    function removeListeners() {
        Schema.resize.off(resizeMaps);
        Schema.redraw.off(resizeMaps);
    }

    function addListeners() {
        Schema.resize.on(resizeMaps);
        Schema.redraw.on(resizeMaps);
    }
    ...
    function getState(el, data) {
        ...
        var mapDraggable = (Schema.env.touch && data.disableTouch) ? false : true;
...
/**
 * ----------------------------------------------------------------------
 * Schema: Google+ widget
 */
Schema.define('gplus', function ($) {
    //'use strict';
...
/**
 * ----------------------------------------------------------------------
 * Schema: Smooth scroll
 */
Schema.define('scroll', function ($) {
    //'use strict';
...
/**
 * ----------------------------------------------------------------------
 * Schema: Auto-select links to current page or section
 */
Schema.define('links', function ($, _) {
    //'use strict';

    var api = {};
    var $win = $(window);
    var designer;
    var inApp = Schema.env();
    ...
    function init() {
        designer = inApp && Schema.env('design');
        slug = Schema.env('slug') || location.pathname || '';
...
/**
 * ----------------------------------------------------------------------
 * Schema: Slider component
 */
Schema.define('slider', function ($, _) {
    //'use strict';
    ...
    function removeListeners() {
        Schema.resize.off(renderAll);
        Schema.redraw.off(api.redraw);
    }

    function addListeners() {
        Schema.resize.on(renderAll);
        Schema.redraw.on(api.redraw);
    }
...
Schema.define('lightbox', function ($, _) {
    //'use strict';
...
/**
 * ----------------------------------------------------------------------
 * Schema: Navbar component
 */
Schema.define('navbar', function ($, _) {
    //'use strict';
    ...
    function removeListeners() {
        Schema.resize.off(resizeAll);
    }

    function addListeners() {
        Schema.resize.on(resizeAll);
    }
    ...
    function navigate(data) {
        return function (evt) {
            ...
            // Avoid late clicks on touch devices
            if (!Schema.validClick(evt.currentTarget)) {
                evt.preventDefault();
                return;
            }
 ...
 */
Schema.define('dropdown', function ($, _) {
    //'use strict';
...
/**
 * ----------------------------------------------------------------------
 * Schema: Tabs component
 */
Schema.define('tabs', function ($, _) {
    //'use strict';
    ...
    function init() {
        design = inApp && Schema.env('design');
...
/**
 * ----------------------------------------------------------------------
 * Schema: Brand pages on the subdomain
 */
Schema.define('branding', function ($, _) {
    //'use strict';
    ...
    api.ready = function () {
        var doBranding = $html.attr("data-wf-status") && location.href.match(/schema.com|schematest.com/);

        if (doBranding) {
            var $branding = $('<div></div>');
            var $link = $('<a></a>');
            $link.attr('href', 'http://schema.com');
