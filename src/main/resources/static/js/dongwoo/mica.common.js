jQuery("head").append("<script src='../../../js/dongwoo/ajaxService.js'>");
jQuery("head").append("<script src='../../../js/dongwoo/mica.config.js'>");
...
var micaCommon = {
    ...
    jqGrid: {
        ...
        editForm: function (grid, url, mtype, options) {
            ...
            if (rowId != null) {
            	...
            	micaCommon.messageBox({ title: micaCommon.lang("L_Edit"), type: "warning", width: "400", height: "130", html: "Please Select Row" });
    ...
    lang: (function (args) {
        var f = function (str, replace) {
            ...
            micaConfig.lang = sessionStorage.LANGUAGE;
            result = result ? result[micaConfig.lang || micaConfig.defaultLang] : str;
            result = result || str;

            if(replace != null){
            	$.each(replace, function(index, value){
            		var replaceValue = micaCommon.lang.list[value];
            		if(replaceValue == null){
            			replaceValue = value;
            		}else{
            			replaceValue = micaCommon.lang.list[value][micaConfig.lang|| micaConfig.defaultLang];
            		}
    ...
    grid: {
        ...
        langFormOptions: function (options) {
            ...
            //        bExit : "Cancel",
            // form
            options.addCaption = micaCommon.lang("L_Add");
            options.editCaption = micaCommon.lang("L_Edit");
            options.bSubmit = micaCommon.lang("bSubmit");
            options.bCancel = micaCommon.lang("bCancel");
            options.bClose = micaCommon.lang("L_Close");
            ...
            options.delMsg = options.delMsg || micaCommon.lang("M_SuccessDelete"); // del MSG;
            options.msg = micaCommon.lang("M_AreYouSureYouWantToDelete"); // del MSG;  // 삭제하시겠습니까?
            //options.msg = openMes.lang("MU0015");// del MSG;
            options.delChildMsg = micaCommon.lang("delChildMsg"); // del MSG;
            options.caption = micaCommon.lang("L_Delete"); // del 레코드 추가.

            // options.selMsg = micaCommon.lang("selMsg");
            options.selMsg = micaCommon.lang("M_NotSelectedRow");
            if (options.msg == null) {
                options.msg = {};
            }
            //form
        ...
        langGridSet: function (data, gridOptions, options) {
            ...
            $.each(gridOptions.colModel, function (i, v) {
                ...
                // var str = micaCommon.lang(v.name);
        ...
        editForm: function (grid, url, mtype, options) {
            ...
            if (rowId != null) {
                ...
                micaCommon.messageBox({ title: "Edit Form", type: "warning", width: "400", height: "130", html: selMsg });
                //alert(selMsg);
            }
        ...
        delForm: function (grid, url, mtype, options) {
            ...
            if (rowId != null) {
                ...
                var selMsg = options.selMsg || "Please Select Row";
                micaCommon.messageBox({ title: micaCommon.lang("L_Delete"), type: "warning", width: "400", height: "130", html: selMsg });
                //alert(selMsg);
            }
    ...
    treeGrid: {
        ...
        delForm: function (grid, url, mtype, options) {
            ...
            if (rowId != null) {
                ...
                var selMsg = options.selMsg || "Please Select Row";
                micaCommon.messageBox({ title: "Delete Form", type: "warning", width: "400", height: "130", html: selMsg });
                //alert(selMsg);
            }
            ...
            */
            ...
            if (dataOptions.url) {
                ajaxService(dataOptions.data, dataOptions.url, dataOptions.successFn, dataOptions.mtype, dataOptions.errorFn, options).done(function (data) {
                    ...
                    micaCommon.leftMenu.makeMenu(el, record);
                });
            }
                ...
                if (typeof level1.items != 'undefined' && level1.items.length > 0) {
                    ...
                    if (null != level1.HREF && typeof level1.HREF != 'undefined' && level1.HREF.length > 0) {
                        if ("PAGE" == micaCommon.leftMenu.linkType.toUpperCase()) {
                            menuLvl1.on('click', function () {
                            	var locObj = micaCommon.fncS.keyValueSet({data:JSON.parse(sessionStorage.MENU), key:"VIEW_ID"})[level1.HREF] || {};
                            	var reqParam = {
        		    			...
        		    			var options = {
        		    				...
        		    				success: function (data) {
        		    					if(data.isAuth=="N"){
        		    						...
        		    						micaCommon.messageBox({ title: "No permission", type: "warning", width: "400", height: "110", html: micaCommon.lang("M_NotAuthorization") });
        		    						return;
        		    					}
                    ...
                    $.each(level1.items, function (index2, level2) {
                        ...
                        if (typeof level2.items != 'undefined' && level2.items.length > 0) {
                            ...
                            if (null != level2.HREF && typeof level2.HREF != 'undefined' && level2.HREF.length > 0) {
                                if ("PAGE" == micaCommon.leftMenu.linkType.toUpperCase()) {
                                    lvl2Dropdown.on('click', function () {
                                    	var locObj = micaCommon.fncS.keyValueSet({data:JSON.parse(sessionStorage.MENU), key:"VIEW_ID"})[level2.HREF] || {};
                                    	var reqParam = {
                		    				menu_id : locObj.MENU_ID,
                		    				menu_class_id : locObj.MENU_CLASS_ID
                		    			};
                        ...
                        if (typeof level2.items != 'undefined' && level2.items.length > 0) {
                            ...
                            $.each(level2.items, function (index3, level3) {
                                ...
                                if ("TAB" == micaCommon.leftMenu.linkType.toUpperCase()) {
                                	...
                                	menuLvl3.on('click', function () {
                                		var locObj = micaCommon.fncS.keyValueSet({data:JSON.parse(sessionStorage.MENU), key:"VIEW_ID"})[level3.HREF] || {};
                                		var reqParam = {
                		    				menu_id : locObj.MENU_ID,
                		    				menu_class_id : locObj.MENU_CLASS_ID
                		    			};
    ...
    splash: {
        set: function (el) {
            ...
            var imgSplash = $('<img/>', {
                ...
                src: micaConfig.loadingImage,
                id: 'imgSplash'
            });
            $('#micaSplash').append(imgSplash);
        },
        show: function ($el) {
            $('#micaSplash').remove();
            this.set($el);
    ...
    page: {
        getCurrentPageInfo: function (key, separator, targetUrl) {
            ...
            */
            ...
            var locUrl = targetUrl || micaCommon.pageUrl || location.href.split(separator).pop().split("?")[0].split("#")[0];
            var tempUrl = null;
            var sameVal = null;
            //1레벨 같은 URL 체크
