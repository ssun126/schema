        ...
        styleUI : {
            jQueryUI : {
                ...
                base : {
                    ...
                    icon_first: "ui-icon fa fa-angle-double-left bigger-140",
                    icon_prev: "ui-icon fa fa-angle-left bigger-140",
                    icon_next: "ui-icon fa fa-angle-right bigger-140",
                    icon_end: "ui-icon fa fa-angle-double-right bigger-140",
                    /*	01.04 khp			icon_first : "ui-icon-seek-first",
    ...
    $.fn.jqGrid = function( pin ) {
        ...
        return this.each( function() {
            ...
            var sortkeys = ["shiftKey","altKey","ctrlKey"],
                ...
                addJSONData = function (data, gridDIV, rcnt, more, adjust) {
                    var startReq = new Date();
                    if(data) {
                        if(ts.p.treeANode === -1 && !ts.p.scroll) {
                            emptyRows.call(ts, false, true);
                    ...
                    var afterInsRow = $.isFunction(ts.p.afterInsertRow), grpdata=[],hiderow=false, groupingPrepare,
                        ...
                        rnc = ni ? getstyle(stylemodule, 'rownumBox', false, 'jqgrid-rownum') :"",
                        mlc = gi ? getstyle(stylemodule, 'multiBox', false, 'cbox') : "";
                    //KHP			$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first");
                    var tablebody = $("tbody:first", gridDIV);
                    if(ts.p.grouping)  {
                        hiderow = ts.p.groupingView.groupCollapse === true;
                        groupingPrepare = $.jgrid.getMethod("groupingPrepare");
                    }
                    ...
                    if(!ts.grid.hDiv.loading) {
                        ...
                        switch(dt)
                        {
                            ...
                            case "mica": //khp
                                ...
                                $(ts).triggerHandler("jqGridAfterLoadComplete", [data]);
                                if (pvis) { ts.grid.populateVisible(); }
//				        if (ts.p.loadonce || true) { ts.p.datatype = "local"; }
                                if (npage === 1) { endReq(); }
                                /*
                beginReq();
...
*/
                ...
                setPager = function (pgid, tp){
                    // TBD - consider escaping pgid with pgid = $.jgrid.jqID(pgid); khp
                    var sep = "<td class='ui-pg-button1 "+disabled+"'><span class='ui-separator'></span></td>",
                        pginp = "",
...
//Helper functions
        ...
        createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
            ...
            mhDiv.className = "widget-header ";
            $(mhDiv).append("<span class='ui-jqdialog-title'>" + p.caption + "</span>");
            ...
            $(mw).css($.extend({
                ...
                height: "auto",
//01.04	khp		height:isNaN(p.height) ? "auto" : p.height + "px",
                zIndex: p.zIndex,
                overflow: 'hidden'
            },coord))
    ...
    $.jgrid.extend({
        editGridRow : function(rowid, p){
            ...
            return this.each(function(){
                ...
                var bP = "<a id='" + bp + "' class='fm-button " + commonstyle.button + "'><span class='ui-icon ui-icon-triangle-1-w' style='display: none;'></span><i class='ace-icon fa fa-chevron-left'></i></a>",
                    bN = "<a id='" + bn + "' class='fm-button " + commonstyle.button + "'><span class='ui-icon ui-icon-triangle-1-e' style='display: none;'></span><i class='ace-icon fa fa-chevron-right'></i></a>",
                    //<span class='ui-icon ui-icon-triangle-1-w' style='display: none;'></span><i class='ace-icon fa fa-chevron-left'></i>
                    //<span class='ui-icon ui-icon-triangle-1-e' style='display: none;'></span><i class='ace-icon fa fa-chevron-right'></i>
                    //bS = "<a id='sData' class='fm-button " + commonstyle.button + "'>" + p.bSubmit + "</a>",
                    //bC  ="<a id='cData' class='fm-button " + commonstyle.button + "'>"+p.bCancel+"</a>";
                    bS = '<a id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm btn-primary"><i class="ace-icon fa fa-check"></i>' + p.bSubmit + '</a>',
                    bC = '<a id="cData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm"><i class="ace-icon fa fa-times"></i>'+p.bCancel+'</a>';
                ...
                if(p.saveicon[0]===true) {
                    $("#sData",frmtb2).addClass(p.saveicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
                        .append("<span class='"+commonstyle.icon_base + " " +p.saveicon[2]+"' style='display:none;'></span>");
                }
                if(p.closeicon[0]===true) {
                    $("#cData",frmtb2).addClass(p.closeicon[1] === "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
                        .append("<span class='" + commonstyle.icon_base + " " + p.closeicon[2] + "'style='display:none;'></span>");
                }
                if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
...
//01.04 khp
                    var bS = '<a id="dData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm btn-primary"><i class="ace-icon fa fa-check"></i>' + p.bSubmit + '</a>',
                        bC = '<a id="eData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm"><i class="ace-icon fa fa-times"></i>' + p.bCancel + '</a>';
            ...
            return this.each(function(){
                ...
                $.each(grp.groups,function(i,n){
                    ...
                    if(grp.groupSummaryPos[n.idx] === 'header')  {
                        str += "<tr id=\"" + hid + "\"" + (grp.groupCollapse && n.idx > 0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"" + common.content + " jqgroup ui-row-" + $t.p.direction + " " + clid + "\"><td style=\"padding-left:10px;" + "\"" + mul + ">" + icon + grpTextStr + "</td>";
//khp 01.05				    str += "<tr id=\"" + hid + "\"" + (grp.groupCollapse && n.idx > 0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"" + common.content + " jqgroup ui-row-" + $t.p.direction + " " + clid + "\"><td style=\"padding-left:" + (n.idx * 12) + "px;" + "\"" + mul + ">" + icon + grpTextStr + "</td>";
    ...
    $.jgrid.extend({
            ...
            return this.each(function(){
                ...
                while(i<len) {
                    ...
                    if(ldat[expanded] === false) {
                        twrap += ((ldat[isLeaf] === true) ? "'" : $t.p.treeIcons.plus+" tree-plus fa fa-caret-right treeclick'");
                    } else {
                        twrap += ((ldat[isLeaf] === true) ? "'" : $t.p.treeIcons.minus+" tree-minus fa fa-caret-down treeclick'");
                    }
        ...
        reloadNode: function(rc) {
            return this.each(function(){
                ...
                rc[expanded] = true;
                $("div.treeclick",rc1).removeClass(this.p.treeIcons.plus+" tree-plus fa fa-caret-right").addClass(this.p.treeIcons.minus+" tree-minus fa fa-caret-down");
        ...
        collapseNode : function(rc) {
            return this.each(function(){
                ...
                if(rc[expanded]) {
                    ...
                    rc[expanded] = false;
                    if( ret === false ) { return; }
                    $("div.treeclick",rc1).removeClass(this.p.treeIcons.minus+" tree-minus fa fa-caret-down").addClass(this.p.treeIcons.plus+" tree-plus fa fa-caret-right");
                ...
                if ( parentid === undefined  || parentid === null || parentid==="") {
                    ...
                    if(parentdata[isLeaf]) {
                        ...
                        $($t.rows[prow])
                            ...
                            .end()
                            .find("div.tree-leaf").removeClass($t.p.treeIcons.leaf+" tree-leaf").addClass($t.p.treeIcons.minus+" tree-minus fa fa-caret-down");
